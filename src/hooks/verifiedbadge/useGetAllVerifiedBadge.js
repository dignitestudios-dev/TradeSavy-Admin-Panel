...
const useGetAllVerifiedBadge = ({
  ...
  const getVerifiedBadge = async (params = {}) => {
    setLoading(true);
    try {
      const {
        search: s = search,
      ...
      const response = await api.AllVerifiedBadge(query);

      if (response?.success) {
        setVerifiedBadge(response?.data?.users || []);
        setPagination(
...
export default useGetAllVerifiedBadge;
