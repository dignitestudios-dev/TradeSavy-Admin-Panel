...
console.log(ordertotalData, "ordertotalDataordertotalData");
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Product",
      render: (products) => (
        <Badge variant="outline">{products?.[0]?.product?.name || "N/A"}</Badge>
      ),
    },
    {
      key: "totalAmount",
  ...
  const handleOrderExport = (orders) => {
    return orders.map((order) => ({
      ...
      // 👇 OBJECT → STRING
      "Renter Name": order.renter?.fullName || "",
      "Renter Email": order.renter?.email || "",

      "Lender Name": order.lender?.fullName || "",
      "Lender Email": order.lender?.email || "",
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <DataTable
          title="Reports Generations"
          loading={loadingOrders}
          data={orders}
          columns={columns}
          currentPage={currentPage}
          pageSize={pageSize}
          totalData={ordertotalData || 0} 
...
export default ReportsGenerations;
